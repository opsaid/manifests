...
[Addon Store 公共规范](../../../docs/spec/addon-store.md)。规范正文只维护在该引用文件中。
本技能不授予部署、发布或修改只读资源的额外权限，沿用用户在当前任务中的授权。

## 工作流程

1. 检查工作区已有改动，读取目标应用入口、被引用的配置与资源，以及 `docs/apps/<app>/README.md`
   （存在时）。区分公共默认值、环境差异、凭据及应用结构变化。
2. 公共默认值维护在 `addons/<app>/`；真实差异维护在私有仓库 overlay；
   `examples/overlays/<app>/` 只保存完整的虚构环境示例。具体接口与发布要求以公共规范为准。
3. 修改时同步应用输入说明与受影响的示例；若规范改变，仅更新 `docs/spec/addon-store.md` 中的权威文本。
   功能资源的必要变更依照 AGENTS.md 处理，不以本技能为由扩大改动范围。
4. 执行根 `scripts/validate.sh`，并对已注册合同的目标应用执行 `scripts/validate.sh --app <app>`；
   未注册合同的存量应用使用 `--app <app> --build-only`，不得据此声称完成商店验收。
   首期合同检查仅实现 open-webui，新增应用在 `scripts/checks/` 补对应检查并注册。修改校验器时运行
