...
- 公共仓库的 addon、template、example、文档和可访问历史中不得包含真实租户私有配置或凭据。
  公共上游域名、项目身份等不是租户秘密；示例使用 `example.com` 等保留示例域和明确占位符。
...
- 旧应用可以分批迁移，但公开仓库前必须检查全部发布内容和可访问历史。
...
应用 ID 一旦发布应保持稳定；版本来自 `<id>-v*` 发布 tag 和 CHANGELOG，镜像版本来自对应 ref
...
`examples/overlays/<id>/`，包含所有被引用文件，只使用虚构值和占位凭据。
...
- CHANGELOG 每次发布记录模板版本、应用版本、重要默认值变化、输入变化、数据迁移和已验证环境。
