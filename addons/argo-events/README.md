...
上游项目：[argoproj/argo-events](https://github.com/argoproj/argo-events)。
...
RBAC 已按 Kind 拆分；controller-config.yaml 是 files generator 原始输入，位于 configuration/configmaps/。
`clusters/crds/crds.yaml` 保留同 Kind 的上游多对象 CRD 清单，避免无功能收益的格式重写。

迁移前后 17 个渲染对象内容一致；没有变更对象名称、镜像版本、权限、配置或存储行为。

```bash
scripts/validate.sh --app argo-events --build-only
```
